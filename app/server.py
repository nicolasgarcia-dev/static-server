...
app = FastAPI(
    ...
    description="Servidor de archivos HTML estáticos con panel de gestión interactivo.",
    version="1.0.0",
    docs_url="/_docs",
    redoc_url="/_redoc",
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path.startswith(("/_static", "/_manager", "/api")) or request.url.path == "/":
...
def render_dashboard(request: Request):
    ...
    rendered = template.render(
        request=request,
        storage_dir_name=STORAGE_DIR.name
    )
...
async def get_index(request: Request):
    """
    Main entry point: Displays the interactive dashboard.
    """
...
async def serve_stored_file(full_path: str, request: Request):
    """
    Directly serve stored HTML files and assets based on relative URL path.
    Example: GET /CIM/fichajes-cim.html -> serves html_storage/CIM/fichajes-cim.html
    """
    # Exclude internal routes
    if full_path.startswith(("_static", "_docs", "_redoc", "_openapi", "api")):
    ...
    if not target_path.exists():
        # Clean 404 response with strict XSS sanitization of full_path
    ...
    if target_path.is_dir():
        ...
        if index_file.exists():
            ...
            # If requesting a directory without index.html, redirect or open explorer at that path
            return render_dashboard(request)
